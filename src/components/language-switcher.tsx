    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button variant="outline" size="icon" className="h-9 w-9 cursor-pointer" aria-label="Switch Language">
            ...
            <DropdownMenuContent align="end" className={"mt-1"} style={{ backgroundColor: "var(--background)" }}>
                {languages.map((language) => (
                    <DropdownMenuItem
                        ...
                        className={currentLanguage === language.code ? "bg-muted font-medium cursor-pointer" : "cursor-pointer"}
                    >
                        {language.name}
                    </DropdownMenuItem>
                ))}
